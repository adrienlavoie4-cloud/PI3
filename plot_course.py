...
def lisser_course(df):
    ...
    historique_pitch_median = deque(
        maxlen=FENETRE_MEDIANE_PITCH
    )

    moyenne_pitch = puissance_estimee.MoyenneMobile(
        fenetre=FENETRE_MOYENNE_PITCH
    )

    moyenne_puissance = puissance_estimee.MoyenneMobile(
        fenetre=FENETRE_PUISSANCE
    )
    ...
    for ligne in df.itertuples(index=False):
        ...
        if PITCH_MIN_DEG <= pitch_brut <= PITCH_MAX_DEG:
            ...
            pitch_median = median(
                historique_pitch_median
            )

            if (
                dernier_pitch_limite is None
                or dernier_temps_pitch is None
            ):
                ...
                variation_max = (
                    VITESSE_VARIATION_PITCH_MAX
                    * delta_t
                )

                variation_demandee = (
                    pitch_median
                    - dernier_pitch_limite
                )
            ...
            dernier_pitch_valide = (
                moyenne_pitch.ajouter(pitch_limite)
            )
            dernier_temps_pitch = temps_s

        # Si le pitch est invalide, on conserve la dernière
        # valeur filtrée au lieu d'utiliser la valeur injectée.
        if dernier_pitch_valide is None:
            dernier_pitch_valide = 0.0

        pitch_resultat.append(
            dernier_pitch_valide
        )
        ...
        acceleration_utilisee = (
            derniere_acceleration_valide
        )

        acceleration_resultat.append(
            acceleration_utilisee
        )
        ...
        vitesse_utilisee = (
            derniere_vitesse_valide
        )

        vitesse_resultat.append(
            vitesse_utilisee
        )
        ...
        if not vitesse_valide:
            ...
            puissance_resultat.append(
                derniere_puissance_affichee
            )
            continue
        # ------------------------------------------------------
        ...
        vitesse_ms = (
            puissance_estimee.kmh_vers_ms(
                vitesse_utilisee
            )
        )

        pitch_rad = (
            puissance_estimee.degres_vers_radians(
                dernier_pitch_valide
            )
        )
        ...
        composantes = (
            puissance_estimee.estimer_puissance(
                mesures
            )
        )

        puissance_roue_signee = (
            composantes.total_roue_w
        )
        ...
        puissance_roue_lissee = (
            moyenne_puissance.ajouter(
                puissance_roue_signee
            )
        )
        ...
        puissance_pedalier = (
            puissance_estimee.puissance_pedalier(
                puissance_roue_positive
            )
        )
...
def main():
    if len(sys.argv) < 2:
        print(
            "Usage : python plot_course.py "
            "course.csv [--recalculer]"
        )
        sys.exit(1)
    ...
    recalculer = (
        "--recalculer" in sys.argv
        or "-recalculer" in sys.argv
    )
